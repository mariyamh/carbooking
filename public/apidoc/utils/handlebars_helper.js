define(['locales', 'handlebars', 'diffMatchPatch'], (locale, Handlebars, DiffMatchPatch) => {
  /**
  ...
  Handlebars.registerHelper('markdown', (text) => {
    ...
    text = text.replace(/((\[(.*?)\])?\(#)((.+?):(.+?))(\))/gm, (match, p1, p2, p3, p4, p5, p6) => {
      const link = p3 || `${p5}/${p6}`;
   ...
   *
   */
  Handlebars.registerHelper('gen_body', (context, options) => {
    const strBody = {};
   ...
   *
   */
  Handlebars.registerHelper('each_compare_field', (source, compare, options) =>
    _handlebarsEachCompared('field', source, compare, options)
  );

  /**
   *
   */
  Handlebars.registerHelper('each_compare_title', (source, compare, options) =>
    _handlebarsEachCompared('title', source, compare, options)
  );
