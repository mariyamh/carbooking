...
function init($, _, locale, Handlebars, apiProject, apiData, Prism, sampleRequest, semver) {
  ...
  function sortFields(fields_object) {
    $.each(fields_object, (key, fields) => {
      ...
      const objects = fields.filter((item) => item.type === 'Object');
      ...
      for (var object of objects) {
        ...
        const objectFields = fields.filter((item) => item.field.indexOf(`${object.field}.`) > -1);
      ...
      const nonObjects = fields.filter((item) => item.field.indexOf('.') === -1 && item.type !== 'Object');
